...
<link rel="stylesheet" href="/common/assets/css/board.css">
<!-- GOOGLE FONTS -->
<link
						...
						<div class="col-md-8 community_layout">
											...
											<c:forEach items="${map.list }" var="row">
												...
												<tr>
													...
													<td class="title subject"><a
														href="javascript:fn_openBoardDetail('${row.RV_NUM }', '${map.paging.curPage }', '${map.searchType}', '${map.searchWord }')">
															<span> <strong> ${row.RV_TITLE } </strong>
														</span> <input type="hidden" id="RV_NUM" value="${row.RV_NUM }">
													</a></td>
													<td class="subject_nickname">${row.MB_ID }</td>
													<td class="subject_count">${row.RV_HIT }</td>
													<td class="subject_date">${row.RV_DATE }</td>
												</tr>
											</c:forEach>
										</c:when>
										<c:otherwise>
							...
							<div class="table_control">
								<div class="table_write">
									<div class="write_btn">
										<a href="#this" class="btn" id="write1"> <span> <strong>
													글쓰기 </strong>
										</span>
										</a>
									</div>
