						...
						<div class="col-md-12">
							<div class="panel panel-headline">
								<div class="panel-heading">
									<div class="panel-heading">
										<h3 class="panel-title">제휴업체 등록/관리</h3>
									</div>
								</div>
								<div class="panel-body">
											...
											<c:choose>
												<c:when test="${fn:length(map.list) > 0}">
													<c:forEach items="${map.list }" var="row">
														<tr>
															<td><a
																href="javascript:fn_openBoardDetail('${row.GY_NUM }', '${map.paging.curPage }', '${map.searchType}', '${map.searchWord }')">
																	${row.GY_NAME } </a> <input type="hidden" id="GYM_NUM"
																value="${row.GY_NUM }"></td>
															<td>${row.GY_ADDR1 }&nbsp;${row.GY_ADDR2 }</td>
															<td>${row.GY_PHONE }</td>
															<td>${row.GY_CNAME }</td>
														</tr>
													</c:forEach>
									...
									<div class="table_control">
										<div class="table_write">
											<div class="write_btn">
												<a href="#this" class="btn" id="writegym">등록</a>
											</div>
										</div>
												...
												<select name="searchType" id="searchType">
													<c:choose>
														<c:when test="${map.searchType == 'NAME' }">
															...
															<option value="COACH">코치이름</option>
															<option value="ADDR">주소</option>
														</c:when>
														<c:when test="${map.searchType == 'ADDR' }">
															<option value="NAME">상호명</option>
															<option value="COACH">코치이름</option>
															<option value="ADDR" selected="selected">주소</option>
														</c:when>
														<c:when test="${map.searchType == 'COACH' }">
															<option value="NAME">상호명</option>
															<option value="COACH" selected="selected">코치이름</option>
															<option value="ADDR">주소</option>
														</c:when>
													</c:choose>
												</select>
