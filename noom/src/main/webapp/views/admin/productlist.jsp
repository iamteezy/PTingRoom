						...
						<div class="col-md-12">
							<div class="panel panel-headline">
								...
								<div class="panel-body">
									<!-- 코드 넣는곳 -->
									<table class="table table-striped" summary="상품관리">
										<colgroup>
											<col width="20%" />
											<col width="20%" />
											<col width="50%" />
											...
											<c:choose>
												<c:when test="${fn:length(map.list) > 0}">
													<c:forEach items="${map.list }" var="row">
														<tr>
															<td><a
																href="javascript:fn_openPdUpdate('${row.PD_NAME }', '${map.paging.curPage }', '${map.searchType}', '${map.searchWord }')">
																	${row.PD_NAME } </a> <input type="hidden" id="PD_NAME"
																value="${row.PD_NAME }"></td>
															<td>${row.PD_PRICE }</td>
															<td>${row.PD_BODY }</td>
															<td>
																<div class="btns pddelete">
																	<a href="" id="pddelete" value="${row.PD_NAME }"
																		class="btn">삭제</a>
																</div>
															</td>
														</tr>
										...
										<div class="table_search">
												...
												<select name="searchType" id="searchType">
													<c:choose>
														<c:when test="${map.searchType == 'NAME' }">
															...
															<option value="BODY">설명</option>
														</c:when>
														<c:when test="${map.searchType == 'BODY' }">
															<option value="NAME">상품명</option>
															<option value="BODY" selected="selected">설명</option>
														</c:when>
													</c:choose>
												</select>
											</div>
											<div class="search_input">
												<input type="text" name="searchWord" id="searchWord"
													...
													class="btn search_btn" id="pd_search">검색</a>
