						...
						<div class="col-md-12">
							<div class="panel panel-headline">
								<div class="panel-heading">
									<div class="panel-heading">
										<h3 class="panel-title">FAQ 등록/관리</h3>
									</div>
								</div>
								<div class="panel-body">
									<!-- 코드 넣는곳 -->
									<table class="table table-striped" summary="FAQ관리">
										<colgroup>
											<col width="20%" />
											<col width="80%" />
										</colgroup>
												...
												<c:when test="${fn:length(map.list) > 0}">
													<c:forEach items="${map.list }" var="row">
														<tr class="off" id="mFAQ_${row.FAQ_NUM }">
															<td>${row.FAQ_NUM }<input type="hidden" id="FAQ_NUM"
																name="FAQ_NUM" value="${row.FAQ_NUM }" />
															</td>
															<td><a
														...
														<tr id="dFAQ_${row.FAQ_NUM }" style="display: none">
															<td colspan="2" style="white-space: pre-wrap;"
																class="answer"><div>${row.FAQ_BODY }</div> <c:set
																	var="ADM_ID" value="${row.ADM_ID }" /> <c:set
																	var="session" value="${admloginInfo.ADM_ID }" /> <c:if
																	test="${ADM_ID eq session}">
																	<div class="table_control2 indetail" style="white-space: nowrap;">
																		<div class="btns">
																			<a
																				href="javascript:fn_openBoardUpdate_f(${row.FAQ_NUM })"
																				class="btn">수정하기</a>
																				</div>
																				<div class="btns">
																			<a
																				href="javascript:fn_deleteBoard_f(${row.FAQ_NUM })"
																				class="btn">삭제하기</a>
																		</div>
																		</div> 
																</c:if></td>
									...
									<div class="table_control">
										<div class="table_write">
											<div class="write_btn">
												...
												<c:if test="${session eq 'admin'}">
													<a href="#this" class="btn" id="f_write1">글쓰기</a>
												</c:if>
											</div>
										</div>
										<div class="table_paging">
											...
											<c:if test="${map.paging.curPage>1 }">
												<a href="javascript:fn_list_f('1')">[처음]</a>
											</c:if>
											<!-- 이전 페이지 블록으로 이동 : 현재 페이지 블럭이 1보다 크면 [이전]을 출력	 -->
											<c:if test="${map.paging.curBlock>1 }">
												<a href="javascript:fn_list_f('map.paging.prevPage')">[이전]</a>
											</c:if>
										...
										<div class="table_search">
											...
											<div class="search_input">
												<input type="text" name="searchWord" id="searchWord"
													...
													class="btn search_btn" id="f_search">검색</a>
											</div>
