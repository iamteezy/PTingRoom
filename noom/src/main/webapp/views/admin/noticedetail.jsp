						...
						<div class="col-md-12">
							<div class="panel panel-headline">
								<div class="panel-heading">
									<div class="panel-heading">
										<h3 class="panel-title">공지사항상세보기</h3>
									</div>
								</div>
								<div class="panel-body">
									...
									<form id="frm">
										<table class="table table-striped">
											...
											<tbody>
												<tr>
													...
													<td class="revise">
														${NTMAP.NT_NUM }
														<input type="hidden" id="NT_NUM" name="NT_NUM" value="${NTMAP.NT_NUM }" >
													</td>
													<th scope="row" >작성일</th>
													<td class="revise">${NTMAP.NT_DATE }</td>
												</tr>
												<tr>
													<th scope="row">제목</th>
													<td colspan="3" class="revise">${NTMAP.NT_TITLE }</td>
													<input type="hidden" id="curPage" value="${NTMAP.curPage}" />
													<input type="hidden" id="searchType" value="${NTMAP.searchType}" />
													<input type="hidden" id="searchWord" value="${NTMAP.searchWord}" />
												</tr>
												<tr>
												<th scope="row">본문</th>
													<td colspan="3" style="white-space:pre-wrap;">${NTMAP.NT_BODY }</td>
										...
										<c:set var="ADM_ID" value="${NTMAP.ADM_ID }" />
										<c:set var="session" value="${admloginInfo.ADM_ID }" />
										<c:if test="${ADM_ID eq session}">
										<div class="btns">
											<a href="#this" class="btn" id="update1">수정하기</a>
											</div>
											<div class="btns">
